...
- 将选中行设置为对应的标题（一级 ~ 六级）
- 文字常见的样式设置：加粗（`**`）、倾斜(`*`)、下划线（`<u></u>`）、删除线（`~~`）、引用（`>`）、上标（`<sup>`）、下标（`<sub>`）
- 文字对齐：左对齐、右对齐、居中对齐、两侧对齐
- 插入页面分隔符（`------------`）、段内换行
- 有序列表、无序列表、任务列表
- 插入行内代码、代码块
- 插入行内公式、公式块
- 插入 mermaid、plantuml 图片代码
- 插入链接、插入图片、表格
- Emoji 表情图标和特殊字符已支持输入
- Material Admonition 样式

详细实现参考 [020-构建编辑器工具栏](../020-构建编辑器工具栏.md)
