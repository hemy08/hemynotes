...
flowchart TD
    subgraph Windows [Windows]
		direction TB
        A1[应用程序访问内存] --> B1[CPU生成虚拟地址]
        B1 --> C1[内存管理单元MMU<br/>页表查找]
        C1 --> D1[页表项PTE状态]
        D1 --> E1{状态判断}
        E1 -->|页在内存| F1[正常继续执行]
        E1 -->|页不在内存<br/>缺页异常| G1[缺页异常处理]
        E1 -->|页未提交<br/>访问违规| H1[访问违规错误]
        H1 --> I1
        G1 --> I1[从磁盘加载页<br/>或分配物理页]
        I1 --> J1[更新页表项]
        J1 --> K1[重新执行指令]
    end
    ...
    subgraph Linux [Linux]
		direction TB
        A2[应用程序访问内存] --> B2[CPU生成虚拟地址]
        B2 --> C2[内存管理单元MMU<br/>多级页表查找]
        C2 --> D2[页表项PTE状态]
        D2 --> E2{状态判断}
        E2 -->|页在内存| F2[正常继续执行]
        E2 -->|页不在内存<br/>缺页异常| G2[缺页异常处理]
        E2 -->|零页或<br/>Copy-on-Write| H2[按需分页]
        G2 --> I2[分配物理帧<br/>零初始化]
        H2 --> I2
        I2 --> J2[更新页表项]
        J2 --> K2[重新执行指令]
...
stateDiagram-v2
    Windows --> 空闲状态
    空闲状态 --> 保留状态: VirtualAlloc<br/><a/><b/>MEM_RESERVE
    保留状态 --> 提交状态: VirtualAlloc<br/><a/><b/>MEM_COMMIT
    提交状态 --> 空闲状态: VirtualFree<br/><a/><b/>MEM_RELEASE
    空闲状态 --> 提交状态: 首次访问/缺页异常

    Linux --> 未映射状态
    未映射状态 --> 已映射状态: brk/mmap
    已映射状态 --> 已分配状态: 首次访问/缺页异常
    已分配状态 --> 未映射状态: munmap/brk
```
...
flowchart TD
    subgraph app [应用层]
        A1[应用程序] --> B1[free<span>ptr</span>]
        A2[应用程序] --> B2[free<span>ptr</span>]
    end

    subgraph crt [CRT层/glibc层]
        B1 --> C1[CRT内存释放器<br/>判断内存类型]
        C1 --> D1{内存类型}
        D1 -->|小内存| E1[HeapFree]
        D1 -->|大内存| F1[VirtualFree]
        B2 --> C2[glibc ptmalloc<br/>判断chunk类型]
        C2 --> D2{分配类型}
        D2 -->|bin回收| E2[返回bin池<br/>- fastbin<br/>- smallbin<br/>- largebin]
        D2 -->|mmap分配| F2[munmap释放<br/>直接解除映射<br/>返回给系统]
    end

    subgraph sys [系统层]
        E1 --> G1[返回堆空闲列表<br/>可能合并相邻块]
        F1 --> H1[释放虚拟地址空间<br/>解除物理页映射]
        F2 --> G2
        E2 --> G2[可能的合并操作<br/>前后空闲块]
        G2 --> H2[top chunk<br/>可能brk收缩]
    end
```
...
```mermaid
graph TB
    A[Linux内存架构] --> B[Arena分配域]
    A --> C[Bins空闲链表]
    A --> D[mmap区域]
    A --> E[页缓存]
    
    B --> B1[per-thread arena]
    B --> B2[main arena]
...
flowchart TD
	...
	subgraph B [分配阶段]
		...
		subgraph Windows1 [Windows]
			...
			WH[CRT分配器] --> WI{大小判断}
			WI -->|小内存| WJ[HeapAlloc]
			WI -->|大内存| WK[VirtualAlloc]
		end
		subgraph Linux1 [Linux]
			direction TB
			LH[ptmalloc] --> LI{大小判断}
			LI -->|小内存| LJ[fast/small bin]
			LI -->|中内存| LK[large bin]
			LI -->|大内存| LL[mmap]
		end
	end
	...
	subgraph F [释放阶段]
		direction TB
		subgraph Windows3 [Windows]
			direction TB
			WR[CRT释放器] --> WS{内存类型}
			WS -->|小内存| WT[HeapFree]
			WS -->|大内存| WU[VirtualFree]
		end
		subgraph Linux3 [Linux]
			direction TB
			LS[ptmalloc] --> LT{分配类型}
			LT -->|bin内存| LU[返回bin池]
			LT -->|mmap内存| LV[munmap]
