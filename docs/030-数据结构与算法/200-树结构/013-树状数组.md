...
!!! note "核心优势"
    代码极其简洁（核心仅需10行左右），常数因子小，实际运行效率往往优于线段树。适用于单点修改+区间查询、区间修改+单点查询等场景。
...
!!! note "建树优化"
    传统建树需要 O(n log n)，利用父子关系可优化到 O(n)。对于每个节点，将其值累加到父节点。
...
!!! note "选择建议"
    如果只需要单点修改+区间查询，优先选择树状数组（代码简洁、效率高）；如果需要区间修改、区间最值等复杂操作，选择线段树。
