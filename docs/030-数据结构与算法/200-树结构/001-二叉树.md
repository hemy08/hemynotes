...
对任何非空二叉树，若 $n_0$ 为叶子节点数，$n_2$ 为度数为2的节点数，则：
...
!!! note "验证示例"
    ...
    $n_0 = n_2 + 1 \Rightarrow 3 = 2 + 1$ ✓

### 性质4：完全二叉树的深度

n 个节点的完全二叉树深度为 $\lfloor \log_2 n \rfloor + 1$

!!! note "验证"

    | 节点数 n | 公式 | 深度 |
    |----------|------|------|
    | n=1 | $\lfloor \log_2 1 \rfloor + 1 = 0 + 1$ | 1 ✓ |
    | n=3 | $\lfloor \log_2 3 \rfloor + 1 = 1 + 1$ | 2 ✓ |
    | n=7 | $\lfloor \log_2 7 \rfloor + 1 = 2 + 1$ | 3 ✓ |

### 性质5：完全二叉树的节点编号

对完全二叉树按层序编号，对任一编号为 i 的节点：
...
!!! note "编号关系验证"
    ...
    - 父节点(1)的子节点: 左 $= 2 \times 1 = 2$, 右 $= 2 \times 1 + 1 = 3$
    - 节点(2)的父节点: $\lfloor 2/2 \rfloor = 1$
    - 节点(5)的父节点: $\lfloor 5/2 \rfloor = 2$
...
!!! note "链式存储结构"
    ```mermaid
    graph LR
        N1[("Node 1: data=1, left→, right→]
        N2[("Node 2: data=2, left→, right→]
        N3[("Node 4: data=4, left=NULL, right=NULL")]
