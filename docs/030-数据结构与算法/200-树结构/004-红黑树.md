...
!!! note "删除修复的四种情况"
    ...
    | 情况 | 条件 | 处理方式 |
    |------|------|----------|
    | **情况1** | 兄弟是红色 | 重新着色并旋转，转为兄弟是黑色的情况 |
    | **情况2** | 兄弟是黑色，且两个子节点都是黑色 | 将兄弟着为红色，双黑上移到父节点 |
    | **情况3** | 兄弟是黑色，远侄子是红色，近侄子是黑色 | 旋转并重新着色，转为情况4 |
    | **情况4** | 兄弟是黑色，远侄子是红色 | 旋转并重新着色，消除双黑 |
```
...
| 特性 | AVL树 | 红黑树 | B树 |
|------|-------|--------|-----|
| **平衡程度** | 严格平衡（高度差 ≤ 1） | 近似平衡（最长路径 ≤ 2×最短路径） | 多路平衡 |
| **高度上界** | 1.44 × log₂(n) | 2 × log₂(n+1) | log_m(n)（m为阶数） |
| **查找效率** | 最优 | 次优 | 磁盘友好 |
| **插入旋转** | 最多1次 | 最多2次 | 分裂操作 |
| **删除旋转** | 最多 O(log n) 次 | 最多3次 | 合并操作 |
| **适用场景** | 查找密集型应用 | 插入删除频繁 | 磁盘存储优化 |
| 应用场景 | 内存查找 | 通用 | 磁盘存储 |
...
!!! note "Linux CFS (Completely Fair Scheduler) 调度器"
