...
graph TB
    ...
    subgraph "push(20), push(30) 后 (top = 2)"
        C0["10"]:::filled
        C1["20"]:::filled
        C2["30"]:::top
        C3["空"]:::empty
    ...
    classDef empty fill:#F5F5F5,stroke:#CCC,stroke-dasharray:5
    classDef filled fill:#E3F2FD,stroke:#2196F3
    classDef top fill:#E8F5E9,stroke:#4CAF50
```
...
- 数组实现：$O(n)$，其中 $n$ 为栈的最大容量
- 链表实现：$O(n)$，其中 $n$ 为栈中元素个数
