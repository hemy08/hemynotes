...
!!! note "核心特征"
    优先队列的出队顺序由元素的优先级决定，而非入队顺序。通常使用**堆（Heap）**来实现，保证高效的插入和删除操作。
...
!!! note "建堆时间复杂度分析（自底向上方法）"
    ...
    **总时间**:
    $$T(n) = \sum_{k=0}^{h} 2^{h-k} \times k = n \times \sum_{k=0}^{h} \frac{k}{2^k} < n \times \sum_{k=0}^{\infty} \frac{k}{2^k} = n \times 2 = O(n)$$
