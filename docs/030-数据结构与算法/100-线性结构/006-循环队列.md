...
!!! warning "普通队列的假溢出示例"
    ...
    **Step 3: 想要入队 6,7**
    问题: rear已经到数组末尾，无法继续入队

    但是前面有空位！这就是"假溢出"——实际空闲位置: 3个 (索引0,1,2) 但无法利用！
...
| 入队 | $O(1)$ | 直接操作rear指针 |
| 出队 | $O(1)$ | 直接操作front指针 |
| 获取队首 | $O(1)$ | 直接访问 data[front] |
| 判空/判满 | $O(1)$ | 比较指针或计数器 |
| 获取大小 | $O(1)$ | 计算或直接返回count |

## 空间复杂度

$O(\text{capacity})$ - 预分配固定大小数组
