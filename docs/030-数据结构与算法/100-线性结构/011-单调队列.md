...
!!! note "核心特征"
    单调队列维护窗口内的**候选最值元素**，通过在入队时移除无效元素，保证队首始终是当前窗口的最值。每个元素最多入队出队一次，因此整体时间复杂度为 O(n)。
...
graph LR
    ...
    style A5 fill:#E3F2FD,stroke:#2196F3
...
| 队列类型 | 特点 |
|----------|------|
| 普通队列 | 保持FIFO顺序，不保证任何有序性 |
| 单调队列 | 队首是当前窗口最值，元素单调递减 |
