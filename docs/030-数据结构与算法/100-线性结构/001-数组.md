...
```mermaid
graph TB
    subgraph "数组 arr[6] = {10, 20, 30, 40, 50, 60}"
        A["地址: 1000"] 
        B["地址: 1004"] 
        C["地址: 1008"] 
        D["地址: 1012"] 
...
!!! tip "基地址"
    索引0的地址称为基地址(base address)，通过 `base + i × size` 可以计算任意索引i的元素地址。
...
插入/删除操作需要移动后续所有元素，时间复杂度为 $O(n)$。

| 操作位置 | 时间复杂度 | 说明 |
|----------|------------|------|
| 尾部插入/删除 | $O(1)$ | 无需移动元素 |
| 头部插入/删除 | $O(n)$ | 移动所有元素 |
| 中间插入/删除 | $O(n)$ | 平均移动 $n/2$ 个元素 |
...
| 插入元素 | $O(1)$ | $O(n)$ | $O(n)$ | 尾部插入为 $O(1)$ |
| 删除元素 | $O(1)$ | $O(n)$ | $O(n)$ | 尾部删除为 $O(1)$ |
...
|------|------------|------|
| 静态数组 | $O(n)$ | 编译时确定大小 |
| 动态数组 | $O(n)$ | 运行时可扩容，实际分配 capacity |
| 均摊分析 | $O(n)$ | 扩容均摊代价为 $O(1)$ |
...
!!! note "扩容代价分析"
    ...
    **扩容序列**: $1 \rightarrow 2 \rightarrow 4 \rightarrow 8 \rightarrow 16 \rightarrow 32 \rightarrow ... \rightarrow n$
    
    **复制总次数**: $1 + 2 + 4 + 8 + ... + n = 2n - 1$
    
    **均摊每次插入的复制代价**: $O(1)$

虽然单次扩容需要 $O(n)$ 时间复制元素，但扩容频率越来越低，均摊到每次插入操作，代价仅为 $O(1)$。
...
双指针是一种常用的数组优化技巧，可以将 $O(n^2)$ 的问题优化到 $O(n)$。
...
对于二维数组 `matrix[row][col]`，地址计算公式为：

$$address = base\_address + (row \times cols + col) \times element\_size$$
