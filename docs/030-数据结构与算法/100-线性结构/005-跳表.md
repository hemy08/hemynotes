...
!!! note "空间复杂度分析"
    ...
    每个节点的期望指针数 $= 1 + \frac{1}{1-P} = 1 + 2 = 3$ (当 $P=0.5$)
    
    **总空间** $= n \times \text{平均指针数} = O(n)$
    
    **详细计算**:
    $$E[\text{指针数}] = \sum_{k=0}^{\infty} P(\text{层数} \geq k) = \sum_{k=0}^{\infty} P^k = \frac{1}{1-P}$$
